...
title: Exact-cone tiled bed geometry, two bed states (fluidized/collapsed), mass-conserving TRISO packing
status: complete — exact-cone tiled adopted as production 2026-09-16; staircase retired
---

## What is implemented (production)

The bed is a single OpenMC cell whose region is the true frustum
(`-cone_surf & +zp_cone_bot & -zp_cone_top_plane`) filled by a broadcast tile
lattice. Particles are placed by packing one small cubic tile once at the
mass-conserving packing fraction and replicating that shared universe across
the bed bounding box via a `RectLattice`. Unique-cell count is O(particles per
tile), independent of charge mass, so the same geometry runs at 5 g and at
3610 g without memory scaling.
...
- `furnace/geometry.py`:
  - `exact_cone_bed(params, state, stage, background_material, charge_mass_g,
    ...
    the frustum cell, computes the mass-conserving tile pf, and handles
    overflow into the retort cylinder if `V_bulk > V_frustum`.
...
1. **Cell region is the true cone.** `frustum_region = -cone_surf & +zp_cone_bot
   & -zp_cone_top_plane`. The `ZCone` surface is the actual cell boundary; no
   inscribed staircase, no annular voids, no per-slab discretisation.

2. **Mass-conserving tile density.** Given
   `target_n = charge_mass_g / m_particle`, the tile is packed at
   `pf_tile = min(pf_state, target_n × v_sphere / v_frustum)`.
   When `V_bulk < V_frustum` (all nominal cases at 95 g), this reduces the tile
   density so the total-in-frustum particle count matches `target_n`. When the
   charge exceeds the frustum, `pf_tile = pf_state` and the excess spills into
   a retort-cylinder overflow cell handled in the same function.

3. **Tiled fill.** `tiled_bed()` packs one ~0.5 cm cubic tile once (cached,
   keyed on stage / pf / seed), wraps it in a `Universe`, and every position
   of an outer `RectLattice` covering `(-r_retort, -r_retort, 0) →
   (r_retort, r_retort, z_cone_top)` holds *the same universe object*. Unique
   cell count is set by particles per tile (~1600 at bare_kernel, pf ≈ 0.5),
   not by bed volume.

4. **Boundary clipping.** TRISO particles whose centres sit inside the cone
   but whose outer shells poke past `cone_surf` are geometrically clipped by
   the cell region. The inside portion remains a TRISO sub-cell; the outside
   portion is not part of `frustum_cell` and is replaced by whatever material
   the surrounding cell holds (graphite retort wall). Affected fraction < 1 %
   of fissile mass; measured effect on k-eff is at MC-noise level
   (|Δk|/σ_seed ≈ 1.7 at 5 g in the tiled-vs-random validation).

5. **No transport outside the cone.** The cell region is the outer boundary.
   When a neutron crosses `cone_surf` going outward it exits `frustum_cell`
   entirely and enters whichever cell surrounds it — the tile universe is
   never queried at positions outside the cone. The `RectLattice.outer`
   fallback exists as a safety net but is unreachable from inside the frustum
   cell.
...
- **Mass-conserving tile pf.** Packing the frustum at `pf_state` regardless of
  charge mass over-produces particles whenever `V_bulk < V_frustum`. Scaling
  `pf_tile` by `target_n × v_sphere / v_frustum` preserves fissile inventory
  and matches the semantics of the random exact-cone reference. See commit
  `eec253d`.
...
- **Fluidized bed occupies cone + cylinder overflow at pf_fluidized, not
  cylinder only.** Uniform fluidization distributes particles throughout the
  available volume at the fluidized packing fraction. Placing all particles
  above the cone top would model a spouted-bed regime, not the operating
  regime of this furnace.
...
- **Nominal 95 g charge stays within the cone at both packing states.** At
  PF = 0.50 (collapsed), bare kernels give V_bulk = 18.10 cm³ < V_frustum ≈
  28.2 cm³. At PF = 0.333 (fluidized), V_bulk = 27.17 cm³ < V_frustum by a
  narrow margin. Overflow into the retort cylinder is exercised by the
  step-7 mass sweep (up to 3610 g).
