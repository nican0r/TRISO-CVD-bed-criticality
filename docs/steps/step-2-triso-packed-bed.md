...
- **Thin-slab / narrow-cylinder RSP guard in `pack_bed`.** `pack_spheres` uses force-biased packing (FBP) for pf ≥ ~0.30. FBP degenerates in two geometries: (a) near-monolayer slabs (h < 2.5d) where particle centres have almost no vertical freedom, and (b) narrow cylinders (r < 15 particle radii) where near-wall ordering pushes the achievable random packing fraction toward pf=0.50 and FBP stalls. Both were encountered during production cache warm-up for the n_slabs=32 staircase: the last partial slab of the nominal 95 g collapsed bed has h = 0.064 cm = 1.5d (trigger a), and slab 0 at the cone throat has r/r_particle = 14.12 (trigger b). The guard detects either condition, extends the region to 5d tall with pf_ext = pf × z_orig / (5d), and calls `pack_spheres` on the taller region; pf_ext ≤ pf × 2.5d / 5d = 0.25 across the full trigger range (worst case: z=2.5d, pf=0.50) — safely in RSP territory. Centres are then filtered back to the original z-band. The guard fires only when 5d > z_extent; for taller slabs (z ≥ 5d) FBP converges normally and the extension is skipped. The cache is written under the original bounding-box key. **Known non-conservatism:** after z-filtering, the affected slab retains ≈ z_ext / (5d) of the target particle count (~30% for the 95 g / n_slabs=32 partial slab at h=1.5d; ~28% for slab 0 in full_triso stage runs where the threshold spans slabs 0–4). This is a <0.2% deficit of total bed inventory and is accepted as a Stage 0 screening limitation.
